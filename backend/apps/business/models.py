...
class Scale(models.Model):
    ...
    BITS_NUMBER_CHOICES = [(i, str(i)) for i in [5,6,7,8, 16, 32, 64, 128, 256, 512, 1024]]
    ...
    connection_type = models.CharField( max_length=10, choices=CONNECTION_TYPE_CHOICES, default='tcp')
    ...
    #Serial Connection
    serial_port = models.CharField(max_length=10, choices=SERIAL_PORT_CHOICES, blank=True, null=True)
    baudrate = models.IntegerField(choices=BAUDRATE_CHOICES, default=9600, blank=True, null=True)
    parity = models.CharField(max_length=10, choices=PARITY_CHOICES, blank=True, null=True)
    stop_bits = models.FloatField(choices=STOP_BITS_CHOICES, blank=True, null=True)
    flow_control = models.CharField(max_length=20, choices=FLOW_CONTROL_CHOICES, blank=True, null=True)
