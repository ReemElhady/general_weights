...
const AddVehicleModal = ({ onClose }) => {
    ...
    useEffect(() => {
        const fetchVehicles = async () => {
            try {
                const data = await driverAPI.get({ page_size: 5000, ordering: 'id' });
                const results = Array.isArray(data) ? data : data.results || [];
                setDrivers(results);
            } catch (err) {
                error(err.message, "فشل في تحميل السائقين");
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            error(err.message, "فشل في حفظ السيارة");
        }
    };
    ...
    return ReactDOM.createPortal(
        <div dir="rtl" className="fixed inset-0 z-50 bg-black bg-opacity-50 flex justify-center items-center">
            ...
            <div
                ...
                className="relative z-50 bg-white rounded-[12px] w-[645px] max-h-[90vh] flex flex-col shadow-xl"
            >
                <div className="overflow-y-auto scrollbar-hide max-h-[calc(100vh-150px)] p-6">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-lg font-semibold">إضافة سيارة</h2>
